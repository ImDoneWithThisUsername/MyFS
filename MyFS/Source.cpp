...
void init_FAT(int S_B, int S_F, int N_F, LPCWSTR fileName) {
	...
	BYTE* clus_RDET = dec_to_hex_with_little_endian(EOF, 4);		//0x0FFFFFFF
	for (int i = 0; i < 4; i++) {
		buffer[i] = start[i];
		buffer[i + 4] = next_start[i];
		buffer[i + 8] = clus_pass[i];
		buffer[i + 12] = clus_RDET[i];
	}
	for (int i = 0; i < N_F; i++) {
...
bool savePassword_file(string password, int offset, int startPos, LPCWSTR fileName) {
	bool flag;
	BYTE* byte_pass = string_to_hex(password);//chuyen password thanh byte
...
bool savePassword_volume(string password, int offset, LPCWSTR fileName) {
	...
	BYTE* byte_pass = string_to_hex(hashed_password); //chuyen password than byte
